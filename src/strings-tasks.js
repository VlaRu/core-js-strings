...
function repeatString(str, times) {
  return times < 0 ? '' : str.repeat(Math.ceil(times));
}
...
function countVowels(/* str */) {
  ...
  return Array.from(lowercaseStr).reduce((count, char) => {
...
function isPalindrome(str) {
  ...
  return alphanumericStr.split('').reverse().join('') === alphanumericStr;
}
...
function extractNameFromTemplate(/* value */) {
  ...
  const name = words.slice(2, 4).join(' ');
  return name; */
  throw new Error('Not implemented');
}
...
function encodeToRot13(str) {
  return str.replace(/[a-zA-Z]/g, function encodeTo(char) {
